...
public class TypeTransformer {
    ...
    public static Expression T (Expression e, TypeMap tm) {
        ...
        if (e instanceof Unary) {
			...
			if (typ.equals(Type.INT))        // -int
				return new Unary(u.op.intMapU(u.op.val), t);
			else if (typ.equals(Type.FLOAT)) // -float
				return new Unary(u.op.floatMapU(u.op.val), t);
    ...
    public static Statement T (Statement s, TypeMap tm) {
        ...
        if (s instanceof Assignment) {
            ...
            if (ttype.equals(Type.FLOAT)) {
                ...
                if (srctype.equals(Type.CHAR)) {
                    ...
                    srctype = Type.INT;
                } else if (srctype.equals(Type.FLOAT)){
        ...
        if (s instanceof Print) {
            Print p = (Print)s;
            Expression source = T(p.source, tm);
            return new Print(source);
        }
        if (s instanceof PrintCh) {
            PrintCh pCh = (PrintCh)s;
            Expression source = T(pCh.source, tm);
            return new PrintCh(source);
        }
        throw new IllegalArgumentException("should never reach here");
    }
    ...
    public static void main(String args[]) {
        ...
        prog.display();           // this is also done in Parser.java
        System.out.println("\nBegin type checking...");
        TypeMap map = StaticTypeCheck.typing(prog.decpart);
        map.display();    // this is also done in StaticTypeChecker.java
        StaticTypeCheck.V(prog);
        Program out = T(prog, map);
        System.out.println("\nTransformed Abstract Syntax Tree\n");
        out.display();    // after transform
    } //main
